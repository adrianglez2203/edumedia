...
if( !function_exists( 'cl_send_email_templates' ) ) {
    ...
    function  cl_send_email_templates( $email_data, $payload = array(), $test_email = false )
    {
        ...
        if( filter_var( $email_data->email, FILTER_VALIDATE_EMAIL ) ) {
            ...
            $checkout_url = cl_get_checkout_page_url( $token_data, $email_data->provider );
    ...
    function cl_check_if_category_exists( $cart_contents, $email_meta )
    {
        ...
        if( isset( $email_meta[ 'coupon_included_products' ] ) ) {
            ...
            foreach( $cart_contents as $cart_content ) {
                ...
                if( count( array_intersect( $cat_ids, $term_list ) ) > 0 ) {
                    return true;
                }
            }
        }
...
if ( !function_exists( 'cl_get_checkout_page_url' ) ) {
	 ...
	 * @return string
	 */
	function cl_get_checkout_page_url( $data, $provider = 'wc' )
	{
		$token = urlencode( base64_encode( http_build_query( $data ) ) );
		if ( $provider === 'wc' && cl_is_wc_active() ) {
			$checkout_url = wc_get_checkout_url() . '?cl_token=' . $token;
			return esc_url( $checkout_url );
		}
		if ( $provider === 'edd' && cl_is_edd_active() ) {
			$checkout_url = edd_get_checkout_uri() . '?cl_token=' . $token;
			return esc_url( $checkout_url );
		}
		if ( $provider === 'lp' && cl_is_lp_active() ) {
			$checkout_url = learn_press_get_checkout_url() . '?cl_token=' . $token;
			return esc_url( $checkout_url );
		}
        return '';
	}
    ...
    function cl_get_cart_data( $offset, $per_page, $search_query )
    {
        ...
        foreach( $results as $result ) {
                            ...
                            try {
                                ...
                                if( $product && $product->is_in_stock() ) {
                                    $products[] = array(
                                        ...
                                        'price'          => wc_price( $item[ 'quantity' ] * $product->get_price() ),
	        ...
	        if ( $result->cart_meta ) {
		        ...
		        $phone         = $cart_meta[ 'phone' ] !== '' ? $cart_meta[ 'phone' ] : __( 'NOT FOUND', 'cart-lift' );
            ...
            $data[] = array(
                ...
                'products'     => $products,
                'email'        => $result->email,
                'cart_meta'    => unserialize( $result->cart_meta ),
...
//            'page' => 1,
        ...
        elseif ( $provider === 'edd' ) {
            $customers = EDD()->customers->get_customers( array( 'email' => $email ) );
            $purchased_ids = array();
            foreach ( $customers as $customer ) {
                $payment_ids = explode( ',', $customer->payment_ids );
                $payments_query = new EDD_Payments_Query( array( 'post__in' => $payment_ids ) );
                $payments       = $payments_query->get_payments();

                foreach ( $payments as $payment ) {
                    $payment_date = strtotime( $payment->date );

                    if ( $payment_date >= $cart_added_time || $payment_date <= strtotime( time() ) ) {
    ...
    function cl_check_if_product_is_ordered( $orders, $abandoned_cart, $provider )
    {
        ...
        if ( $provider === 'wc' ) {
            foreach ( $orders as $order ) {
                ...
                $common_items = array_intersect( $abandoned_ids, $order_ids );
                return count( $common_items ) > 0;
            }
        }
        elseif ( $provider === 'edd' ) {
            $common_items = array_intersect( $abandoned_ids, $orders );
            return count( $common_items ) > 0;
        }
        return false;
    }
