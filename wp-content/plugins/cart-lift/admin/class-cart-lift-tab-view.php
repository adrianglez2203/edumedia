...
class Cart_Lift_Tab_View
{
	...
	public function __construct()
	{
		...
		$this->sub_action       = filter_input( INPUT_GET, 'sub_action', FILTER_SANITIZE_STRING );
	...
	{
		$cart_data_deleted      = filter_input( INPUT_GET, 'cl_cart_data_deleted', FILTER_SANITIZE_STRING );
		$cart_data_bulk_deleted = filter_input( INPUT_GET, 'cl_cart_data_bulk_deleted', FILTER_SANITIZE_STRING );

		$template_created      = filter_input( INPUT_GET, 'cl_template_created', FILTER_SANITIZE_STRING );
		$template_updated      = filter_input( INPUT_GET, 'cl_template_updated', FILTER_SANITIZE_STRING );
		$template_deleted      = filter_input( INPUT_GET, 'cl_template_deleted', FILTER_SANITIZE_STRING );
		$template_bulk_deleted = filter_input( INPUT_GET, 'cl_template_bulk_deleted', FILTER_SANITIZE_STRING );
	...
	public function print_email_tab_contents()
	{
        ...
        elseif ( $this->get_sub_action() == 'save_email_template' ) {
			...
			$form_action = filter_input( INPUT_POST, 'cl_form_action', FILTER_SANITIZE_STRING );
	...
	public function render_email_template_form()
	{
		...
		if ( isset( $template_id ) ) {
			$result      = cl_get_email_template_by_id( $template_id );
			...
			if ( $result ) {
				...
				if ( isset( $result->email_subject ) ) {
					$email_subject = $result->email_subject;
				}
				if ( isset( $result->email_body ) ) {
				...
				if ( isset( $result->amount ) ) {
					$coupon_amount = $result->amount;
				}
				if ( isset( $result->coupon_frequency ) ) {
					$coupon_frequency = $result->coupon_frequency;
				}
				if ( isset( $result->coupon_frequency_unit ) ) {
					$coupon_frequency_unit = $result->coupon_frequency_unit;
				}
				if ( isset( $result->coupon_frequency_unit ) ) {
					$coupon_frequency_unit = $result->coupon_frequency_unit;
				}
				if ( isset( $result->conditional_discount ) ) {
					$enable_coupon_condition = $result->conditional_discount;
				}
				if ( isset( $result->coupon_included_products ) ) {
					if ( !is_array( $result->coupon_included_products ) ) {
						$coupon_included_products = unserialize( $result->coupon_included_products );
					}
					else {
						$coupon_included_products = $result->coupon_included_products;
					}
				}
				if ( isset( $result->coupon_included_products_amount ) ) {
					$coupon_included_products_amount = $result->coupon_included_products_amount;
				}
				if ( isset( $result->free_shipping ) ) {
					$free_shipping = $result->free_shipping;
				}
				if ( isset( $result->individual_use ) ) {
					$individual_use = $result->individual_use;
				}
				if ( isset( $result->coupon_auto_apply ) ) {
					$coupon_auto_apply = $result->coupon_auto_apply;
				}
				if ( isset( $result->coupon_included_categories ) ) {
					if ( !is_array( $result->coupon_included_categories ) ) {
						$coupon_included_categories = unserialize( $result->coupon_included_categories );
					}
					else {
						$coupon_included_categories = $result->coupon_included_categories;
					}
				}
				if ( isset( $result->coupon_included_categories_amount ) ) {
					$coupon_included_categories_amount = $result->coupon_included_categories_amount;
				}
				if ( isset( $result->email_header_text ) ) {
					$email_header_text = $result->email_header_text;
				}
				if ( isset( $result->email_header_color ) ) {
					$email_header_color = $result->email_header_color;
				}
				if ( isset( $result->email_checkout_color ) ) {
					$email_checkout_color = $result->email_checkout_color;
				}
				if ( isset( $result->email_checkout_text ) ) {
					$email_checkout_text = $result->email_checkout_text;
				}
				if ( isset( $result->twilio_sms ) ) {
					$twilio_enabled = $result->twilio_sms == 'enabled' ? 1 : 0;
				}
				if ( isset( $result->twilio_sms_body ) ) {
					$twilio_sms_body = $result->twilio_sms_body;
				}
			}
		}
	...
	public function sanitize_email_fields()
	{
		...
		if ( isset( $_POST[ 'cl_template_name' ] ) ) {
			$sanitized_fields[ 'cl_template_name' ] = filter_input( INPUT_POST, 'cl_template_name', FILTER_SANITIZE_STRING );
		}
		else {
			$sanitized_fields[ 'cl_template_name' ] = '';
		}
		if ( isset( $_POST[ 'cl_email_subject' ] ) ) {
			$sanitized_fields[ 'cl_email_subject' ] = filter_input( INPUT_POST, 'cl_email_subject', FILTER_SANITIZE_STRING );
		}
		else {
			$sanitized_fields[ 'cl_email_subject' ] = '';
		}
		if ( isset( $_POST[ 'cl_email_body' ] ) ) {
			$sanitized_fields[ 'cl_email_body' ] = filter_input( INPUT_POST, 'cl_email_body', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
			$sanitized_fields[ 'cl_email_body' ] = html_entity_decode( $sanitized_fields[ 'cl_email_body' ], ENT_COMPAT, 'UTF-8' );
		}
		else {
		...
		if ( isset( $_POST[ 'cl_email_unit' ] ) ) {
			$sanitized_fields[ 'cl_email_unit' ] = filter_input( INPUT_POST, 'cl_email_unit', FILTER_SANITIZE_STRING );
		}
		else {
		...
		if ( isset( $_POST[ 'cl_twilio_sms_body' ] ) ) {
			...
			$sanitized_fields[ 'cl_twilio_sms_body' ] = filter_input( INPUT_POST, 'cl_twilio_sms_body', FILTER_SANITIZE_STRING );
		}
		else {
	...
	public function sanitize_email_meta_fields()
	{
		...
		$coupon_meta_fields = apply_filters(
			'cl_campaign_coupon_post_data', array(
		...
		$campaign_meta_field = apply_filters(
			'cl_campaign_meta_post_data', array(
